...
-- Step 3: Drop dependent views first (they'll be recreated)
DROP VIEW IF EXISTS member_payment_status CASCADE;
DROP VIEW IF EXISTS current_month_payment_status CASCADE;

-- Step 4: Drop the old integer column
ALTER TABLE users DROP COLUMN IF EXISTS member_id;

-- Step 5: Rename the new column to member_id
...
-- Step 8: Recreate the views with updated member_id column
-- These views may need to be adjusted based on your actual view definitions
-- If you have custom views, you may need to recreate them manually
